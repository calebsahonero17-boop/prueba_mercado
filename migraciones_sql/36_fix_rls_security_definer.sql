-- Borrar la función existente para aplicar la versión con permisos elevados.
DROP FUNCTION IF EXISTS public.productos_por_categoria_para_inicio(uuid);

-- VERSIÓN FINAL (CON SECURITY DEFINER PARA BYPASS RLS)
-- Esta función se ejecuta con los permisos del creador (usualmente un admin),
-- lo que le permite ver todos los productos e ignorar las políticas de RLS
-- que limitan la vista a solo los productos del usuario actual.
...
BEGIN
    ...
    LIMIT 18;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER; -- AÑADIDO CLAVE: Ejecutar con permisos elevados
