...
-- VERSIÓN FINAL (CORRECCIÓN DE AMBIGÜEDAD)
...
BEGIN
    ...
    BEGIN
        ...
        WITH RECURSIVE categoria_descendientes AS (
            SELECT c.id -- Se refiere a categorias.id
        ...
        SELECT 
            p.id, -- Se refiere a productos.id
            p.vendedor_id,
            p.nombre,
            p.descripcion,
            p.precio,
        ...
        WHERE
            ...
            -- CORRECCIÓN: Se añade un alias 'cd' para desambiguar la columna 'id'
    ...
    EXCEPTION
        WHEN OTHERS THEN
            -- Este bloque ayuda a capturar errores internos para depuración.
