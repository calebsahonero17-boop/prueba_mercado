...
function ProductFashionDeals() {
  const navigate = useNavigate();
  const { categories } = useCategories();
  const targetCategory = categories.find(cat => cat.nombre === 'Ropa y accesorios');
  ...
  if (loading) return <div className="text-center py-8">Cargando ofertas de Ropa y accesorios...</div>;
  if (error) return <div className="text-center py-8 text-red-500">Error al cargar ofertas de Ropa y accesorios.</div>;
...
export default ProductFashionDeals;