...
function ProductHomeDeals() {
  const navigate = useNavigate();
  const { categories } = useCategories();
  const targetCategory = categories.find(cat => cat.nombre === 'Hogar y jardín');
  ...
  if (loading) return <div className="text-center py-8">Cargando ofertas de Hogar y Jardín...</div>;
  if (error) return <div className="text-center py-8 text-red-500">Error al cargar ofertas de Hogar y Jardín.</div>;
...
export default ProductHomeDeals;
